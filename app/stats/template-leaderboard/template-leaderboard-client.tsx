    ...
    <Card>
      <CardHeader>
        <div className="flex items-center justify-between gap-2">
          <CardTitle className="text-base">Average Ownership of Entire Squad</CardTitle>
          ...
          <Table>
            <TableHeader>
              <TableRow className="border-white/10">
                ...
                <TableHead className="text-right text-white/60">Avg Ownership %</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {data.map((team, index) => (
                <TableRow key={team.id} className="border-white/10 hover:bg-white/5 cursor-pointer" onClick={async () => {
                  ...
                  <TableCell className="text-right font-bold">{team.averageOwnership.toFixed(2)}%</TableCell>
                </TableRow>
              ))}
            </TableBody>
          </Table>
          ...
          <div className="text-white text-xs">
            ...
            <div className="overflow-y-auto">
              {data.map((team, index) => (
                <div
                  key={team.id}
                  className={`flex items-center px-2 py-1.5 ${index % 2 === 0 ? 'bg-gray-800' : 'bg-gray-900'} cursor-pointer`}
                  onClick={async () => {
                    setModalTeam({ id: team.id, name: team.name, managerName: team.managerName });
                    setModalPlayers([]);
                    setModalLoading(true);
                ...
                >
                  ...
                  <div className="w-16 text-right font-bold">{team.averageOwnership.toFixed(2)}%</div>
                </div>
              ))}
            </div>
          </div>
